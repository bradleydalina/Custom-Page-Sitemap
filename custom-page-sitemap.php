    ...
    */
    ...
    public function init(){
        ...
        */				
		...
		add_action( 'wp_ajax_fmrequest', array($this,"cps_handle_form") );
        add_action( 'wp_ajax_nopriv_fmrequest', array($this,"cps_handle_form") );
		
		add_action( 'wp_ajax_fmdemo', array($this,"cps_handle_demoform") );
        add_action( 'wp_ajax_nopriv_fmdemo', array($this,"cps_handle_demoform") );
	...
	public static function cps_handle_form() {
        ...
        */
        ...
        if ( ! empty( $_POST ) && !isset( $_POST['cps_plugin_action'] ) || !wp_verify_nonce( $_POST['cps_plugin_action'], 'cps_plugin_nonce' )) {
            ?>
	...
	public static function cps_handle_demoform() {		
        ...
        * Verify nonce submitted within the form
        */
        if ( ! empty( $_POST ) && !isset( $_POST['cps_livedemo_action'] ) || !wp_verify_nonce( $_POST['cps_livedemo_action'], 'cps_livedemo_nonce' )) {
            ?>
			<div id="message" class="error notice is-dismissible">
				<p>Sorry, your nonce was incorrect. Please try again.</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button>
			</div>
            ...
            * Validate Post values
            */
            $cps_live_demo =  (isset($_POST['cps_live_demo']) ? $_POST['cps_live_demo'] : '[c_sitemap]');
			$cps_live_demo = stripslashes($cps_live_demo);
				...
				<div class="cps-content">
					...
					<div id="settings-content" class="cps-box hide">
						<ol style="margin-right:15px;">
							<?php
								...
								*/
								if ( ! empty( $_POST ) && isset( $_POST['cps_plugin_action'] ) ) {
									self::cps_handle_form();
								}
							?>
							<form id="cps-settings-form" class="cps-flex-right cps-settings-content" method="POST">
							<?php wp_nonce_field( 'cps_plugin_nonce', 'cps_plugin_action' ); ?>
							<input type="hidden" name="security" value="<?php echo wp_create_nonce( "nonce" ); ?>" />
							<input type="hidden" name="action" value="fmrequest" />
							<input type="hidden" name="cps_plugin_nonce" value=1 />
					...
					<div id="demo-content" class="cps-box hide">
						...
						<form name="live-demo" id="live-demo">							
							<?php wp_nonce_field( 'cps_livedemo_nonce', 'cps_livedemo_action' ); ?>
							<input type="hidden" name="action" value="fmdemo" />
							<input type="hidden" name="cps_livedemo_nonce" value=1 />
							<span class="cps-formgroup">
