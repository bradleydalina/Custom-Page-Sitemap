...
class Custom_Page_Sitemap{
    ...
    */
	...
	public static function cps_clean_input($str=""){
		if(empty($str)){
			return;
		}			
        ...
        */
        if ( ! empty( $_POST ) && !isset( $_POST['cps_plugin_nonce'] ) || !wp_verify_nonce( $_POST['cps_plugin_nonce'], 'cps_plugin_action' ) || !check_ajax_referer( 'cps_plugin_action', 'cps_plugin_nonce' )) {
            ...
            */
            ...
            $cps_title_id =  self::cps_clean_input((isset($_POST['cps_title_id']) ? $_POST['cps_title_id'] : ''));
			$cps_title_class =  self::cps_clean_input((isset($_POST['cps_title_class']) ? $_POST['cps_title_class'] : ''));
			
            $cps_menu_id =  self::cps_clean_input((isset($_POST['cps_menu_id']) ? $_POST['cps_menu_id'] : ''));
            $cps_menu_class =  self::cps_clean_input((isset($_POST['cps_menu_class']) ? $_POST['cps_menu_class'] : ''));
            ...
            $cps_orderby= self::cps_clean_input((isset($_POST['cps_orderby']) ? $_POST['cps_orderby'] : 'name'));
			...
			$cps_recent_posttype=self::cps_clean_input((isset($_POST['cps_recent_posttype']) ? $_POST['cps_recent_posttype'] : ''));
            ...
            $cps_shortcode= self::cps_clean_input((isset($_POST['cps_shortcode']) ? $_POST['cps_shortcode'] : 'c_sitemap' ));
	...
	public static function get_insert_link($str="", $icon=""){
		if(empty($str)){
			return $str;
		}
		$final_array_bylink=[];
		...
		foreach($raw_array_bylink as $index){
			...
			try{
				...
				$final_insert.="<li>$icon<a href=\"{$value}\" {$title}>".self::cps_clean('/[^a-z0-9-_ .()]/mi', '',$obj)."</a></li>";
				}
			}
			catch(Exception $e){
				$raw_insert = "<li>$icon<a href='javascript:(void);' title=\"ex.  p:{&quot;Link Name&quot;:&quot;http://&quot;}\">Invalid Parameter Shortcode - Custom Page Sitemap</a></li>";
			}
