        ...
        ?>
            <ul cps-menu=true order="<?php echo esc_html($args['order']); ?>" <?php echo CustomPageSitemap::cps_clean('/[^a-z0-9_-]/mi', '', $args['menu_class'], '','class="cps-menu '); ?> <?php echo CustomPageSitemap::cps_clean('/[^a-z0-9_-]/mi', '', $args['menu_id'], '', ' id="'); ?>>
             <?php  
				...
				ob_end_clean();
				preg_match('/<a\s[^>].*<\/a>/siU', $categories, $matches);
				for($i=0; $i < count($matches); $i++){
					$categories= str_replace($matches ,sprintf("<li>%s</li>", $matches[$i]), $categories);
				}
				echo str_replace("<br />", (strpos($args['insert_link'],'c:')!==false) ? "<li>".$args['icon'].CustomPageSitemap::get_insert_link($args['insert_link'])['c']."</li>" : '', $categories); 
